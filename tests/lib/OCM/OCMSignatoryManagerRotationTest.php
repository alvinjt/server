...
/**
 * Exercises the Ed25519 key rotation lifecycle (stage / activate / retire) end
 * to end. The test stubs IAppConfig and IdentityProofManager with stateful
 * fakes so we can observe the slot pointer reshuffling and JWKS publication
 * the way a live admin would.
 */
class OCMSignatoryManagerRotationTest extends TestCase {
	...
	public function testJwksBootstrapsActiveKeyOnFirstFetch(): void {
		// Peers must be able to discover us before we have ever signed
		// anything outbound. Hitting /.well-known/jwks.json on a fresh
		// instance therefore has to provision the active key on its own.
		$jwks = $this->signatoryManager->getLocalEd25519Jwks();
		$this->assertCount(1, $jwks);
		$this->assertSame('https://alice.example/ocm#ed25519-1', $jwks[0]->getKid());

		// And the bootstrapped key is the active one for outbound signing.
		$signatory = $this->signatoryManager->getLocalEd25519Signatory();
		$this->assertSame($jwks[0]->getKid(), $signatory->getKeyId());
	}
	...
	public function testFirstCallProvisionsActiveKey(): void {
		...
		$jwks = $this->signatoryManager->getLocalEd25519Jwks();
		$this->assertCount(1, $jwks);
		$this->assertSame($signatory->getKeyId(), $jwks[0]->getKid());
	...
	public function testStageDoesNotChangeActiveSignerButPublishesNewJwk(): void {
		...
		$jwks = $this->signatoryManager->getLocalEd25519Jwks();
		$this->assertSame([$initial->getKeyId(), $staged->getKeyId()], array_map(fn ($jwk) => $jwk->getKid(), $jwks));
	}
	...
	public function testActivatePromotesPendingAndDemotesActive(): void {
		...
		$kids = array_map(fn ($jwk) => $jwk->getKid(), $this->signatoryManager->getLocalEd25519Jwks());
