...
class SignatureManager implements ISignatureManager {
	...
	public function getIncomingSignedRequest(
		...
		// RFC 9421 signs both `Signature` and `Signature-Input` headers; the
		// presence of `Signature-Input` is the unambiguous discriminator
		// against the draft-cavage format which uses `Signature` only.
	...
	/**
	 * RFC 9421 inbound path. The signatory manager MUST implement
	 * {@see IJwkResolvingSignatoryManager} so we can fetch the verification
	 * JWK by keyId; otherwise we fail closed.
	 *
	...
	private function getRfc9421IncomingSignedRequest(
		...
		try {
			$jwk = $signatoryManager->getRemoteJwk($signedRequest->getOrigin(), $signedRequest->getKeyId());
			if ($jwk === null) {
				throw new SignatoryNotFoundException('no JWK resolved for keyid ' . $signedRequest->getKeyId());
			}
			$signedRequest->setJwk($jwk);
