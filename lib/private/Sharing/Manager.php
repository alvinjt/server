...
final readonly class Manager implements IManager {
	...
	public function addShareSource(ShareAccessContext $accessContext, string $id, ShareSource $source): void {
		...
		if (($sourceType = $this->registry->getSourceTypes()[$source->type] ?? null) === null) {
	...
	public function removeShareSource(ShareAccessContext $accessContext, string $id, ShareSource $source): void {
		...
		$this->wrapUpdate($id, function () use ($id, $source): void {
			...
			$qb
				...
				->where($qb->expr()->eq('id', $qb->createNamedParameter((int)$id, IQueryBuilder::PARAM_INT)))
				->andWhere($qb->expr()->eq('source_type', $qb->createNamedParameter($source->type)))
	...
	public function addShareRecipient(ShareAccessContext $accessContext, string $id, ShareRecipient $recipient): void {
		...
		if (($recipientType = $this->registry->getRecipientTypes()[$recipient->type] ?? null) === null) {
	...
	public function removeShareRecipient(ShareAccessContext $accessContext, string $id, ShareRecipient $recipient): void {
		...
		$this->wrapUpdate($id, function () use ($id, $recipient): void {
			...
			$qb
				...
				->where($qb->expr()->eq('id', $qb->createNamedParameter((int)$id, IQueryBuilder::PARAM_INT)))
				->andWhere($qb->expr()->eq('recipient_type', $qb->createNamedParameter($recipient->type)))
	...
	public function updateShareProperty(ShareAccessContext $accessContext, string $id, ShareProperty $property): void {
		...
		if (($propertyType = $this->registry->getProperties()[$property->type] ?? null) === null) {
			throw new ShareInvalidException('The property is not registered: ' . $property->type);
		}
		...
		$this->wrapUpdate($id, function () use ($propertyType, $id, $property): void {
			...
			if ($propertyType instanceof ISharePropertyModifyValue) {
				$qb = $this->connection->getQueryBuilder();
				$qb
					->select('property_value')
					->from('sharing_share_properties')
					->where($qb->expr()->eq('id', $qb->createNamedParameter((int)$id, IQueryBuilder::PARAM_INT)))
					->andWhere($qb->expr()->eq('property_type', $qb->createNamedParameter($property->type)));
			...
			$rowCount = $qb
				...
				->executeStatement();
			if ($rowCount === 0) {
				throw new ShareInvalidException('Tried to update a property that does not exist: ' . $property->type);
			}
		});
	}
	...
	public function updateSharePermission(ShareAccessContext $accessContext, string $id, SharePermission $permission): void {
		...
		if (!isset($this->registry->getPermissions()[$permission->type])) {
			throw new ShareInvalidException('The permission type is not registered: ' . $permission->type);
		}
		...
		$this->wrapUpdate($id, function () use ($id, $permission): void {
			...
			$rowCount = $qb
				...
				->where($qb->expr()->eq('id', $qb->createNamedParameter((int)$id, IQueryBuilder::PARAM_INT)))
				->andWhere($qb->expr()->eq('permission_type', $qb->createNamedParameter($permission->type)))
				->executeStatement();
			if ($rowCount === 0) {
				throw new ShareInvalidException('Tried to update a permission that does not exist: ' . $permission->type);
			}
		});
	}
	...
	public function deleteShare(ShareAccessContext $accessContext, string $id): void {
		...
		// The other tables are cleared due to foreign key constraint and on delete cascade.
	...
	public function listShares(ShareAccessContext $accessContext, ?string $sourceType, ?string $lastShareID, ?int $limit): array {
		return $this->list($accessContext, null, $sourceType, $lastShareID, $limit);
	}
	...
	/**
	 * @param ?class-string<IShareSourceType> $filterSourceType
	 * @return list<Share>
	 * @throws ShareInvalidException
	 */
	private function list(ShareAccessContext $accessContext, ?string $filterShareID, ?string $filterSourceType, ?string $lastShareID, ?int $limit): array {
		$queries = [];
		if ($accessContext->force) {
			...
			if ($recipients !== []) {
				...
				foreach ($recipients as $recipientTypeClass => $recipientValues) {
					$qb->orWhere($qb->expr()->andX(
						$qb->expr()->eq('sr.recipient_type', $qb->createNamedParameter($recipientTypeClass)),
						// TODO: Add chunking
		...
		foreach ($queries as $qb) {
			...
			if ($filterSourceType !== null) {
				$qb->innerJoin('s', 'sharing_share_sources', 'ss', $qb->expr()->andX(
					...
					$qb->expr()->eq('ss.source_type', $qb->createNamedParameter($filterSourceType)),
				));
			}
		...
		$propertyCompatibleSourceTypes = [];
		$propertyCompatibleRecipientTypes = [];
		$registryProperties = $this->registry->getProperties();
		foreach (array_keys($registryProperties) as $propertyClass) {
			$propertyCompatibleSourceTypes[$propertyClass] = $this->registry->getSourceTypesCompatibleWithProperty($propertyClass);
			$propertyCompatibleRecipientTypes[$propertyClass] = $this->registry->getRecipientTypesCompatibleWithProperty($propertyClass);
		}
		...
		foreach ($chunks as $chunk) {
			...
			foreach ($result->fetchAll() as $row) {
				...
				$propertyClass = (string)$row['property_type'];
				if (!isset($propertyCompatibleSourceTypes[$propertyClass], $propertyCompatibleRecipientTypes[$propertyClass])) {
					// Skip properties that are currently not compatible, but don't remove them.
					continue;
				}

				if (array_intersect($propertyCompatibleSourceTypes[$propertyClass], $shareSourceTypes[$id]) === []) {
					// Skip properties that are currently not compatible, but don't remove them.
					continue;
				}

				if (array_intersect($propertyCompatibleRecipientTypes[$propertyClass], $shareRecipientTypes[$id]) === []) {
					// Skip properties that are currently not compatible, but don't remove them.
					continue;
				}
				...
				$property = $registryProperties[$propertyClass];
				if ($property instanceof ISharePropertyModifyValue) {
					$value = $property->modifyValueOnLoad($value);
				}

				$shares[$id]['properties'][] = new ShareProperty($propertyClass, $value);
				$shareProperties[$id] ??= [];
				$shareProperties[$id][$propertyClass] = true;
			}
		}

		foreach (array_keys($shares) as $id) {
			foreach ($registryProperties as $propertyClass => $property) {
				if (
					!isset($shareProperties[$id][$propertyClass])
					&& isset($shareSourceTypes[$id], $shareRecipientTypes[$id])
					&& array_intersect($propertyCompatibleSourceTypes[$propertyClass], $shareSourceTypes[$id]) !== []
					&& array_intersect($propertyCompatibleRecipientTypes[$propertyClass], $shareRecipientTypes[$id]) !== []) {
					$value = $property->getDefaultValue();

					$lastUpdated = $this->wrapUpdate((string)$id, function () use ($id, $propertyClass, $value): void {
						$qb = $this->connection->getQueryBuilder();
						$qb
							->insert('sharing_share_properties')
							->values([
								'id' => $qb->createNamedParameter($id, IQueryBuilder::PARAM_INT),
								'property_type' => $qb->createNamedParameter($propertyClass),
		...
		$permissions = $this->registry->getPermissions();
		$sourceTypePermissions = array_map(
			static fn (array $permissionClasses): array => array_map(
				static fn (string $permissionClass) => $permissions[$permissionClass],
				$permissionClasses,
			),
			$this->registry->getSourceTypePermissions(),
		);
		$permissionSourceType = $this->registry->getPermissionSourceType();
		...
		foreach ($chunks as $chunk) {
			...
			foreach ($result->fetchAll() as $row) {
				...
				/** @var class-string<ISharePermission> $permissionClass */
				$permissionClass = (string)$row['permission_type'];
				if (!isset($permissionSourceType[$permissionClass])) {
					// Skip permissions that are currently not compatible, but don't remove them.
					continue;
				}

				if (!in_array($permissionSourceType[$permissionClass], $shareSourceTypes[$id], true)) {
					// Skip permissions that are currently not compatible, but don't remove them.
					continue;
				}

				$shares[$id]['permissions'][] = new SharePermission($permissionClass, (bool)$row['permission_enabled']);
				$sharePermissions[$id] ??= [];
				$sharePermissions[$id][$permissionClass] = true;
			}
		}
		...
		foreach (array_keys($shares) as $id) {
			...
			foreach ($shareSourceTypes[$id] as $sourceTypeClass) {
				foreach (($sourceTypePermissions[$sourceTypeClass] ?? []) as $permission) {
					if (!isset($sharePermissions[$id][$permission::class])) {
						$enabled = $permission->getDefault() ?? (($category = $permission->getCategory()) !== null && $permissionCategories[$category]->getDefault());

						$lastUpdated = $this->wrapUpdate((string)$id, function () use ($id, $permission, $enabled): void {
							$qb = $this->connection->getQueryBuilder();
							$qb
								->insert('sharing_share_permissions')
								->values([
									'id' => $qb->createNamedParameter($id, IQueryBuilder::PARAM_INT),
									'permission_type' => $qb->createNamedParameter($permission::class),
									'permission_enabled' => $qb->createNamedParameter($enabled, IQueryBuilder::PARAM_BOOL),
								])
								->executeStatement();
						});

						$shares[$id]['permissions'][] = new SharePermission($permission::class, $enabled);
						$shares[$id]['last_updated'] = $lastUpdated;
					}
				}
			}
		...
		if (!$accessContext->force) {
			$filterPropertyTypes = array_filter($this->registry->getProperties(), static fn (IShareProperty $property): bool => $property instanceof ISharePropertyFilter);
